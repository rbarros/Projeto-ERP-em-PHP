...
class VendaItemTipoProdutoAdminChart extends TPage
{
    ...
    public function __construct()
    {
        ...
        $produto_categoria_produto_id = new TDBSelect('produto_categoria_produto_id', 'base_banco', 'CategoriaProduto', 'id', '{nome}','nome asc'  );
        $system_unit_id = new TDBSelect('system_unit_id', 'permission', 'SystemUnit', 'id', '{name}','name asc'  );

        $system_unit_id->setSize('100%', 100);
        $produto_categoria_produto_id->setSize('100%', 100);

        $row1 = $this->form->addFields([new TLabel("Tipo de produto:", null, '14px', null)],[$produto_categoria_produto_id],[new TLabel("Empresa:", null, '14px', null)],[$system_unit_id]);
    ...
    public function onSearch()
    {
        ...
        {
            $filterVar = is_array($data->produto_categoria_produto_id) ? "'".implode("','", $data->produto_categoria_produto_id)."'" : $data->produto_categoria_produto_id;
            $filters[] = new TFilter('produto_id', 'in', "(SELECT id FROM produto WHERE categoria_produto_id in ({$filterVar}))");// create the filter 
        }
        ...
        {
            $filterVar = is_array($data->system_unit_id) ? "'".implode("','", $data->system_unit_id)."'" : $data->system_unit_id;
            $filters[] = new TFilter('venda_id', 'in', "(SELECT id FROM venda_alt WHERE system_unit_id in ({$filterVar}))");// create the filter 
        }
