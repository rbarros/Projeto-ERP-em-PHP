...
class ClientesList extends TPage
{
    ...
    private static $formName = 'form_ClientesList';
    ...
    public function __construct($param = null)
    {
        ...
        $row1 = $this->form->addFields([new TLabel("Id:", null, '14px', null, '100%'),$id],[new TLabel("Tipo pessoa:", null, '14px', null, '100%'),$tipo_pessoa]);
        $row1->layout = [' col-sm-6 col-lg-2','col-sm-2'];

        $row2 = $this->form->addFields([new TLabel("Nome:", null, '14px', null, '100%'),$nome],[new TLabel("CPF:", null, '14px', null, '100%'),$documento],[new TLabel("Email:", null, '14px', null, '100%'),$email],[$estado_id,new TLabel("Estado id:", null, '14px', null, '100%')],[new TLabel("Cidade:", null, '14px', null, '100%'),$cidade_id]);
        $row2->layout = [' col-sm-6 col-lg-4','col-sm-2','col-sm-2','col-sm-2','col-sm-2'];
        ...
        $column_documento = new TDataGridColumn('documento', "CPF", 'left');
        ...
        $column_dt_ativacao_transformed = new TDataGridColumn('dt_ativacao', "Data de ativação", 'left');

        $column_dt_ativacao_transformed->setTransformer(function($value, $object, $row) 
        {
            if(!empty(trim($value)))
            {
                try
        ...
        $this->datagrid->addColumn($column_dt_ativacao_transformed);
        ...
        $dropdown_button_exportar->addPostAction( "CSV", new TAction(['ClientesList', 'onExportCsv'],['static' => 1]), 'datagrid_'.self::$formName, 'fas:table #00b894' );
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Loja","Clientes"]));
        }
        ...
        try
        {
            ...
            if ($objects)
            {
                ...
                foreach ($objects as $object)
                {
                    ...
                    TTransaction::open(self::$database);
                    $estado             = new Estado($object->estado_id);   
                    $object->estado_id  = $estado->uf;
                    TTransaction::close();
