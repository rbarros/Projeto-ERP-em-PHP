...
class ContaPagarAdminReport extends TPage
{
    ...
    public function __construct()
    {
        ...
        $fornecedor = new TDBCombo('fornecedor', 'base_banco', 'Loja', 'id', '{razao_social}','razao_social asc' , $criteria_fornecedor );
        ...
        $multa = new TDBUniqueSearch('multa', 'base_banco', 'Fornecedor', 'id', 'razao_social','razao_social asc'  );

        $quitada->addItems(['t'=>'Sim','f'=>'Não']);
        $multa->setMinLength(2);
        ...
        $row1 = $this->form->addFields([new TLabel("Loja:", null, '14px', null)],[$fornecedor],[new TLabel("Natureza:", null, '14px', null)],[$natureza_id]);
        ...
        $row4 = $this->form->addFields([new TLabel("Quitada:", null, '14px', null)],[$quitada],[new TLabel("Fornecedor:", null, '14px', null)],[$multa]);
        ...
        $container->add(TBreadCrumb::create(["Relatórios","Contas a pagar (administração)"]));
    ...
    public function getFilters()
    {
        ...
        if (isset($data->multa) AND ( (is_scalar($data->multa) AND $data->multa !== '') OR (is_array($data->multa) AND (!empty($data->multa)) )) )
        {

            $filters[] = new TFilter('multa', 'in', $data->multa);// create the filter 
        }
    ...
    public function onGenerate($format)
    {
        try
        {
            ...
            if ($objects)
            {
                $widths = array(200,200,200,200,200,200,200,200,200,200,200,200,200);
                ...
                if (!empty($tr))
                {
                    ...
                    $tr->addStyle('title', 'Helvetica', '10', 'B',   '#ffffff', '#b9736d');
                    $tr->addStyle('datap', 'Arial', '10', '',    '#000000', '#f0f0f0');
                    $tr->addStyle('datai', 'Arial', '10', '',    '#000000', '#ffffff');
                    $tr->addStyle('header', 'Helvetica', '16', 'B',   '#000000', '#904F49');
                    $tr->addStyle('footer', 'Helvetica', '10', 'B',  '#000000', '#F2BEBC');
                    $tr->addStyle('break', 'Helvetica', '10', 'B',  '#ffffff', '#7e5251');
                    $tr->addStyle('total', 'Helvetica', '10', 'I',  '#000000', '#debbb9');
                    $tr->addStyle('breakTotal', 'Helvetica', '10', 'I',  '#000000', '#ffdfdd');
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        if ($object->loja !== $breakValue)
                        {
                            ...
                            $tr->addRow();
                            $tr->addCell($object->render('{entity_column_id:4139603->entity_column_id:3}'), 'left', 'break', 13);
                            $breakTotal = [];
                        }
                        ...
                        $tr->addCell($object->entity_column_id:4139603->entity_column_id:83, 'left', $style);
