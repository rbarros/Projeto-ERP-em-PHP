...
class VendaAdminReport extends TPage
{
    ...
    public function __construct()
    {
        ...
        $container->add(TBreadCrumb::create(["Relatórios","Vendas (administração)"]));
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        if ($object->system_unit_id !== $breakValue)
                        {
                            if (!$firstRow)
                            {
                                ...
                                $breakTotal_id_venda = array_sum($breakTotal['id_venda']);
                                $breakTotal_valor_total = array_sum($breakTotal['valor_total']);

                                $breakTotal_id_venda = call_user_func(function($value)
                                {
                                    if(!$value)
                                    {
                                        $value = 0;
                                ...
                                $tr->addCell('', 'center', 'breakTotal');
                                $tr->addCell('', 'center', 'breakTotal');
                                $tr->addCell($breakTotal_id_venda, 'left', 'breakTotal');
                                $tr->addCell($breakTotal_valor_total, 'left', 'breakTotal');
                            }
                            $tr->addRow();
                            $tr->addCell($object->render('{entity_column_id:4139969->entity_column_id:83}'), 'left', 'break', 7);
                            $breakTotal = [];
                        }
